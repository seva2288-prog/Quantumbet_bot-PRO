...
// ============================================================
// KEEP-ALIVE — приложение само себя будит
// ============================================================
function startKeepAlive() {
    // Пинг каждые 3 минуты (180 секунд)
    keepAliveInterval = setInterval(function() {
        fetch(API_BASE + '/api/all_data?t=' + Date.now())
            .then(response => response.json())
            .then(data => {
                console.log('🔄 Keep-Alive: пинг успешен', new Date().toLocaleTimeString());
                updateKeepAliveStatus('✅', new Date().toLocaleTimeString());
                if (cachedData) {
                    cachedData = data;
                }
                // Дополнительный пинг на /api/keepalive
                fetch(API_BASE + '/api/keepalive?t=' + Date.now()).catch(() => {});
            })
            .catch(err => {
                console.warn('⚠️ Keep-Alive: пинг не удался', err);
                updateKeepAliveStatus('❌', new Date().toLocaleTimeString());
            });
    }, 180000); // 3 минуты

    // Добавляем индикатор в правый нижний угол
    const statusEl = document.createElement('div');
    statusEl.id = 'keepAliveStatus';
    statusEl.textContent = '🔄 Автопинг активен';
...
function updateKeepAliveStatus(icon, time) {
...
// ============================================================
// АВТО-ИМПОРТ X2 ИЗ ЛОГОВ БОТА
// ============================================================
function startAutoImportX2() {
    // Проверяем логи каждые 5 минут
    autoImportInterval = setInterval(function() {
        autoImportX2FromLogs();
    }, 300000); // 5 минут
    
    // Первый импорт через 10 секунд после загрузки
    setTimeout(function() {
        autoImportX2FromLogs();
    }, 10000);
}

async function autoImportX2FromLogs() {
    try {
        console.log('🤖 Авто-импорт X2 из логов...');
        
        // Получаем логи бота
        const response = await fetch(API_BASE + '/api/matches_log?t=' + Date.now());
        if (!response.ok) {
            console.warn('⚠️ Не удалось получить логи');
            return;
        }
        ...
        for (const line of lines) {
            // Ищем строки с "нет мотивации"
            if (line.includes('нет мотивации') || line.includes('no motivation')) {
                // Извлекаем название матча
                const matchMatch = line.match(/([A-Za-z\s]+)\s+vs\s+([A-Za-z\s]+)/i);
                if (!matchMatch) continue;
                
                const home = matchMatch[1].trim();
                const away = matchMatch[2].trim();
                const matchName = home + ' vs ' + away;
                
                // Проверяем, нет ли уже такого матча в X2
                const exists = x2Data.some(m => m.match === matchName && m.date === new Date().toISOString().slice(0,10));
                if (exists) continue;
                
                // Извлекаем XG если есть
                const xgMatch = line.match(/XG:\s*([\d.]+)/i);
                const xg = xgMatch ? parseFloat(xgMatch[1]) : null;
                
                // Определяем фаворита и аутсайдера (по позиции в логе)
                let favorite = home;
                let underdog = away;
                
                // Если есть информация о позициях
                const posMatch = line.match(/H:\s*#(\d+),\s*A:\s*#(\d+)/i);
                if (posMatch) {
                    const homePos = parseInt(posMatch[1]);
                    const awayPos = parseInt(posMatch[2]);
                    if (homePos < awayPos) {
                        favorite = home;
                        underdog = away;
                    } else {
                ...
                const newMatch = {
                    ...
                    result: 'pending',
                    profit: 0,
                    note: 'Авто-импорт из логов бота' + (xg ? ' | XG: ' + xg : '')
                };
                
                x2Data.push(newMatch);
                newMatches++;
            }
        }
        ...
        if (newMatches > 0) {
            saveX2Data();
            showNotification('🤖 Добавлено ' + newMatches + ' X2 матчей из логов!', 'success');
            if (currentPage === 'x2') {
                renderX2Page();
            }
            ...
            console.log('📭 Новых X2 матчей в логах не найдено');
        }
        
    } catch (error) {
        console.error('❌ Ошибка авто-импорта X2:', error);
    }
}
...
// ============================================================
// РЕНДЕР СТРАНИЦЫ АНАЛИЗА
// ============================================================
function renderAnalysis(data) {
    const html = `
        <h2 style="font-size:18px;color:#a78bfa;margin-bottom:4px;">🔍 Анализ пропущенных матчей</h2>
        <div style="color:rgba(255,255,255,0.4);font-size:12px;margin-bottom:10px;">Детальный анализ матчей, которые не прошли фильтр</div>
        
        <div class="card match-analysis-card" style="border:2px solid rgba(167,139,250,0.15);">
            <div class="card-header">
                <h2 style="color:#a78bfa;">📊 Статистика пропущенных матчей</h2>
            ...
            <div id="matchAnalysisStats" style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:12px;">
                <div class="stat-item" style="text-align:center;padding:8px;background:rgba(255,255,255,0.02);border-radius:8px;">
                    <div class="number" style="font-size:22px;font-weight:700;color:#a78bfa;" id="maTotal">0</div>
                    <div class="label" style="font-size:9px;color:rgba(255,255,255,0.3);">Всего матчей</div>
                </div>
                <div class="stat-item" style="text-align:center;padding:8px;background:rgba(255,255,255,0.02);border-radius:8px;">
                    <div class="number" style="font-size:22px;font-weight:700;color:#34d399;" id="maPassed">0</div>
                    <div class="label" style="font-size:9px;color:rgba(255,255,255,0.3);">✅ Пройдено</div>
                </div>
                <div class="stat-item" style="text-align:center;padding:8px;background:rgba(255,255,255,0.02);border-radius:8px;">
                    <div class="number" style="font-size:22px;font-weight:700;color:#f87171;" id="maSkipped">0</div>
                    <div class="label" style="font-size:9px;color:rgba(255,255,255,0.3);">⏭️ Пропущено</div>
                </div>
                <div class="stat-item" style="text-align:center;padding:8px;background:rgba(255,255,255,0.02);border-radius:8px;">
                    <div class="number" style="font-size:22px;font-weight:700;color:#fbbf24;" id="maRate">0%</div>
                    <div class="label" style="font-size:9px;color:rgba(255,255,255,0.3);">📊 Проход</div>
                </div>
            </div>
            
            <div id="matchAnalysisReasons" style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:12px;">
                <div class="reason-item" style="padding:8px;border-radius:8px;border-left:3px solid #f87171;background:rgba(255,255,255,0.02);">
                    <div style="font-size:10px;color:rgba(255,255,255,0.3);">🚫 Нет мотивации</div>
                    <div class="count" style="font-size:18px;font-weight:700;color:#f87171;" id="maNoMotivation">0</div>
                </div>
                <div class="reason-item" style="padding:8px;border-radius:8px;border-left:3px solid #fbbf24;background:rgba(255,255,255,0.02);">
                    <div style="font-size:10px;color:rgba(255,255,255,0.3);">📉 Низкая позиция</div>
                    <div class="count" style="font-size:18px;font-weight:700;color:#fbbf24;" id="maLowPosition">0</div>
                </div>
                <div class="reason-item" style="padding:8px;border-radius:8px;border-left:3px solid #60a5fa;background:rgba(255,255,255,0.02);">
                    <div style="font-size:10px;color:rgba(255,255,255,0.3);">📊 XG вне диапазона</div>
                    <div class="count" style="font-size:18px;font-weight:700;color:#60a5fa;" id="maXGOut">0</div>
                </div>
            </div>
            
            <div id="matchRecommendations" class="recommendations" style="display:none;margin-top:10px;padding:10px;background:rgba(167,139,250,0.05);border-radius:8px;border:1px solid rgba(167,139,250,0.1);">
                <div class="title" style="font-size:11px;color:#a78bfa;font-weight:600;margin-bottom:4px;">💡 Рекомендации</div>
                <div class="list" id="recommendationsList" style="font-size:12px;color:rgba(255,255,255,0.6);"></div>
            </div>
        </div>
    `;
    
    document.getElementById('analysis-content').innerHTML = html;
    setTimeout(loadMatchAnalysis, 300);
}
        ...
        <div class="x2-card">
            <div class="card-header">
                ...
                <div style="display:flex;gap:6px;flex-wrap:wrap;">
                    ...
                    <button class="btn btn-success" onclick="openX2ImportModal()" style="padding:4px 10px;font-size:10px;">📤 Импорт Excel</button>
                    <button class="btn btn-primary" onclick="autoImportX2FromLogs()" style="padding:4px 10px;font-size:10px;">🤖 Из логов</button>
...
// ИМПОРТ X2 ИЗ EXCEL
// ============================================================
function openX2ImportModal() {
    document.getElementById('x2ImportModal').classList.add('active');
    document.getElementById('x2ImportFile').value = '';
    document.getElementById('x2ImportStatus').textContent = '';
}

function closeX2ImportModal() {
    document.getElementById('x2ImportModal').classList.remove('active');
}

function importX2Excel() {
    const fileInput = document.getElementById('x2ImportFile');
    const statusEl = document.getElementById('x2ImportStatus');
    ...
    reader.onload = function(e) {
        try {
            ...
            if (json.length === 0) {
                statusEl.textContent = '❌ Файл пуст!';
                statusEl.style.color = '#f87171';
                return;
            }
            ...
            json.forEach(row => {
                try {
                    const match = row['Матч'] || row['match'] || '';
                    const favorite = row['Фаворит'] || row['favorite'] || '';
                    const underdog = row['Аутсайдер'] || row['underdog'] || '';
                    const odds = parseFloat(row['Кэф X2'] || row['odds'] || 0);
                    const stake = parseFloat(row['Сумма'] || row['stake'] || 0);
                    const date = row['Дата'] || row['date'] || new Date().toISOString().slice(0,10);
                    const score = row['Счёт'] || row['score'] || '-';
                    const result = (row['Результат'] || row['result'] || 'pending').toLowerCase();
                    const note = row['Примечание'] || row['note'] || '';
                    
                    if (!match || !favorite || !underdog) {
                        errors++;
                        return;
                    }
                    
                    // Проверяем на дубликаты
                    const exists = x2Data.some(m => m.match === match && m.date === date);
                    if (exists) {
                        errors++;
                        return;
                    }
            ...
            if (imported > 0) {
                saveX2Data();
                statusEl.textContent = `✅ Импортировано ${imported} матчей! Ошибок: ${errors}`;
                statusEl.style.color = '#34d399';
                showNotification(`✅ Импортировано ${imported} X2 матчей из Excel!`, 'success');
                renderX2Page();
                if (currentPage === 'dashboard') loadPageData('dashboard');
                setTimeout(closeX2ImportModal, 2000);
            } else {
                statusEl.textContent = `❌ Не удалось импортировать матчи. Ошибок: ${errors}`;
                statusEl.style.color = '#f87171';
            }
...
function renderSettings(data) {
    ...
    let html = `
        ...
        <div class="setting-group" style="border:1px solid rgba(52,211,153,0.15);">
            <h2 style="color:#34d399;">🔄 Keep-Alive</h2>
            <div class="setting-item" style="border-bottom:none;">
                <div>
                    <div class="label">Автопинг активен</div>
                    <div class="desc">Приложение само себя будит каждые 3 минуты</div>
                </div>
                <span style="color:#34d399;font-size:11px;">✅ АКТИВЕН</span>
            </div>
        </div>

        <div class="setting-group" style="border:1px solid rgba(251,191,36,0.15);">
            <h2 style="color:#fbbf24;">🤖 Авто-импорт X2</h2>
            <div class="setting-item" style="border-bottom:none;">
                <div>
                    <div class="label">Авто-импорт из логов</div>
                    <div class="desc">Автоматически добавляет X2 матчи из логов бота каждые 5 минут</div>
                </div>
                <span style="color:#34d399;font-size:11px;">✅ АКТИВЕН</span>
            </div>
        </div>
...
window.hideAddMatchModal = hideAddMatchModal;
window.saveManualMatch = saveManualMatch;
window.loadMatchAnalysis = loadMatchAnalysis;
window.resetInteractiveChart = resetInteractiveChart;
...
window.clearAllX2 = clearAllX2;
window.openX2Modal = openX2Modal;
window.closeX2Modal = closeX2Modal;
window.saveX2Match = saveX2Match;
window.editX2Match = editX2Match;
window.deleteX2Match = deleteX2Match;
window.exportX2Excel = exportX2Excel;
window.setX2Filter = setX2Filter;
window.updateInteractiveChart = updateInteractiveChart;
window.openX2ImportModal = openX2ImportModal;
window.closeX2ImportModal = closeX2ImportModal;
window.importX2Excel = importX2Excel;
window.autoImportX2FromLogs = autoImportX2FromLogs;
