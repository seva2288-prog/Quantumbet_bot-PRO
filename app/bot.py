...
# ============================================================
# AUTOBET (ПРЯМОЙ ИМПОРТ БЕЗ ЛЕНИВОЙ ЗАГРУЗКИ)
# ============================================================
try:
...
def export_to_excel():
    ...
    for bet in history:
        ...
        ev = safe_float(bet.get("ev"), 0)
        stake = safe_float(bet.get("stake"), 0)
        result = bet.get("result", "pending")
        profit = safe_float(bet.get("profit"), 0)
...
def get_matches_with_factors():
    ...
    all_leagues = list(Config.LEAGUES) + list(getattr(Config, "CUP_LEAGUES", []))
    ...
    for league_id in all_leagues:
        ...
        try:
            ...
            for match in matches:
                ...
                duplicate = any(
                    isinstance(m, dict) and m.get("fixture", {}).get("id") == match_id
                ...
                match["factors"] = {
                    ...
                    "home_injuries_list": football_api.get_injuries(home_id) or [],
                    "away_injuries_list": football_api.get_injuries(away_id) or [],
                    "home_id": home_id,
                    "away_id": away_id,
    ...
    for bet in bets:
        ...
        for value in values:
            if not isinstance(value, dict):
                continue
            bet_type = str(value.get("value", "")).strip()
            odd = safe_float(value.get("odd"), 0)
            if bet_type and odd > 0:
...
def calculate_adjusted_xg(home_id, away_id, factors):
    ...
    if home_form and len(home_form) > 0:
        ...
        home_form_ratio = home_form_points / (len(home_form) * 3) if len(home_form) > 0 else 0.5
    ...
    if away_form and len(away_form) > 0:
        ...
        away_form_ratio = away_form_points / (len(away_form) * 3) if len(away_form) > 0 else 0.5
    ...
    home_xg *= 1.1
    away_xg *= 0.9
    logger.info("   🏠 Домашнее преимущество: +10% / -10%")
...
def find_top_matches(matches):
    all_matches_data = []
    bets_placed = 0
    max_bets = int(getattr(Config, "MAX_BETS_PER_RUN", 0) or 0)

    logger.info(f"🔍 Анализ {len(matches)} матчей...")

    BET_TYPES = [
        {"type": "under", "label": "ТМ 2.5", "marker": 42.86875, "keys": ["Under 2.5", "Under", "U 2.5"]},
    ...
    for match in matches:
        ...
        if max_bets > 0 and bets_placed >= max_bets:
        ...
        try:
            ...
            if not fixture_id or not isinstance(teams, dict):
                continue
            ...
            if not odds_dict:
                logger.warning(f"⚠️ Нет/не удалось распарсить коэффициенты: {home} vs {away}")
                continue
            ...
            probs = calculate_probabilities(home_xg, away_xg) or {}
            ...
            for bet_config in BET_TYPES:
                ...
                marker = safe_float(bet_config["marker"], 0)

                odds = None
                for key in bet_config["keys"]:
                    if key in odds_dict:
                        odds = safe_float(odds_dict[key], 0)
                        break

                if odds is None or odds <= 1:
                    continue

                prob = safe_float(probs.get(bet_type, 0.33), 0.33)
                if prob > 1:
                    prob /= 100.0
                prob = max(0.0, min(prob, 1.0))

                try:
                    ev = safe_float(calculate_ev(prob, odds), 0)
                except Exception as e:
                    logger.error(f"❌ Ошибка EV для {label}: {e}")
                    continue
                ...
                logger.info(f"   ✅ ДОБАВЛЕНА СТАВКА: {label} | КЭФ: {odds:.2f} | EV: {ev:.2f}%")
            ...
            try:
                ...
                if bet_result:
                    ...
                    bet_match = str(bet_result.get("match", f"{home} vs {away}"))
                    bet_time = bet_result.get("match_time", match_time)
                    bet_name = str(bet_result.get("bet", ""))
                    bet_odds = safe_float(bet_result.get("odds"), odds)
                    bet_stake = safe_float(bet_result.get("stake"), 0)
                    bet_ev = safe_float(bet_result.get("ev"), 0)
                    marker_stake = bet_result.get("marker_stake")

                    msg = f"🤖 <b>АВТО-СТАВКА #{bets_placed}</b>\n"
                    msg += f"🏟️ {bet_match}\n"
                    if bet_time:
                        msg += f"📅 {bet_time}\n"
                    msg += f"📊 {bet_name} | КЭФ: {bet_odds:.2f}\n"
                    msg += f"💰 Сумма: ${bet_stake:.2f}\n"
                    msg += f"📈 EV: {bet_ev:.1f}%"
                    if marker_stake is not None:
                        msg += f"\n🎯 Маркер: ${safe_float(marker_stake):.2f}"

                    send_telegram(msg)
                    logger.info(f"✅ АВТО-СТАВКА #{bets_placed}")

            except Exception as e:
                logger.exception(f"❌ Ошибка авто-ставки: {e}")
                send_error_to_telegram(f"Ошибка авто-ставки: {e}")

        except Exception as e:
            logger.exception(f"❌ Ошибка анализа матча: {e}")
...
def determine_bet_result(bet_type, home_goals, away_goals):
    ...
    bet_type_lower = str(bet_type or "").lower().strip()
    ...
    if "тб 2.5" in bet_type_lower or "over" in bet_type_lower:
        return "win" if total > 2.5 else "loss"

    if "1x" in bet_type_lower or "1х" in bet_type_lower:
        return "win" if home_goals >= away_goals else "loss"

    if "x2" in bet_type_lower or "х2" in bet_type_lower:
...
def update_pending_bets():
    ...
    for bet in history:
        if bet.get("result") not in ("pending", None):
            continue
        ...
        if not fixture_id:
            ...
            if home and away and home != "Unknown" and away != "Unknown":
                try:
                    ...
                    logger.error(f"Ошибка поиска fixture: {e}")
                    fixture_id = None
                if fixture_id:
        ...
        try:
            ...
            logger.error(f"Ошибка определения результата: {e}")
            continue
        ...
        stake = safe_float(bet.get("stake"), 0)
        odds = safe_float(bet.get("odds"), 1)
        ...
        updated += 1
        logger.info(f"✅ Обновлена ставка: {bet.get('home')} vs {bet.get('away')} → {result}")
...
def recalc_stats():
    ...
    total_profit = sum(safe_float(b.get("profit"), 0) for b in history)
    total_stake = sum(safe_float(b.get("stake"), 0) for b in history)
    ...
    stats["winrate"] = round(wins / (wins + losses) * 100, 1) if wins + losses else 0
    stats["roi"] = round(total_profit / total_stake * 100, 1) if total_stake else 0
    ...
    try:
        ...
        if "callback_query" in data:
            ...
            if callback_data.startswith("result_"):
                ...
                if len(parts) >= 3:
                    ...
                    if not match:
                        try:
                            with open(f"data/match_{match_id}.json", "r", encoding="utf-8") as f:
                                match = json.load(f)
                    ...
                    if match and result_type != "skip":
                        ...
                        if bets:
                            best_bet = bets[0]
                            label = str(best_bet.get("label", "")).lower()

                            if result_type == "home":
                                result = "win" if ("1x" in label or "п1" in label) else "loss"
                            elif result_type == "away":
                                result = "win" if ("x2" in label or "п2" in label) else "loss"
                            elif result_type == "draw":
                                result = "win" if ("1x" in label or "x2" in label) else "loss"
                            else:
                                result = "loss"

                            stake = safe_float(best_bet.get("stake"), 0)
                            odds = safe_float(best_bet.get("odds"), 1)
                            ...
                            history.append({
                                ...
                                "odds": odds,
                                "stake": stake,
                                "ev": safe_float(best_bet.get("ev"), 0),
                                "result": result,
                                "profit": profit,
                    ...
                    elif match and result_type == "skip":
                        cache.pop(f"match_{match_id}", None)
                        storage.save_cache(cache)
                        try:
                            os.remove(f"data/match_{match_id}.json")
        ...
        if "message" in data:
            ...
            if text == "/start":
                    ...
                    try:
                        ...
                        if matches:
                            ...
                            if top_matches:
                                ...
                                bets_today_count = getattr(auto_bet, "bets_today", 0) if auto_bet else 0
                        ...
                        logger.exception(f"❌ Ошибка /update: {e}")
                        send_error_to_telegram(f"Ошибка /update: {e}")
                ...
                file, message_text = export_to_excel()
                if file:
                    ...
                    data_form = {"chat_id": Config.ADMIN_CHAT_ID, "caption": "📊 История ставок"}
                    try:
                        requests.post(url, files=files, data=data_form, timeout=30)
                    except Exception as e:
                ...
                team_name = text.replace("/team", "", 1).strip()
                send_telegram(handlers.handle_team(team_name))

            elif text.startswith("/unblock"):
                ip = text.replace("/unblock", "", 1).strip()
                send_telegram(handlers.handle_unblock(ip))

            elif text.startswith("/result"):
                parts = text.replace("/result", "", 1).strip()
                if " vs " in parts:
                    match_parts = parts.rsplit(" ", 1)
                    if len(match_parts) == 2 and "-" in match_parts[1]:
                        send_telegram(handlers.handle_result(match_parts[0], match_parts[1]))
                    else:
...
def api_stats():
    ...
    total_profit = safe_float(stats.get("total_profit"), 0)

    winrate = round(wins / (wins + losses) * 100, 1) if wins + losses else 0
    total_stake = sum(safe_float(bet.get("stake"), 0) for bet in history)
    roi = round(total_profit / total_stake * 100, 1) if total_stake else 0
    avg_stake = round(total_stake / total_bets, 2) if total_bets else 0
...
def api_history():
    ...
    for bet in history:
        item = dict(bet)
        stake = safe_float(item.get("stake"), 0)
        odds = safe_float(item.get("odds"), 1)

        if item.get("result") == "win":
            item["profit"] = round(stake * (odds - 1), 2)
        elif item.get("result") == "loss":
            item["profit"] = -round(stake, 2)
        else:
            item["profit"] = 0.0

        item["match"] = f"{item.get('home', '')} vs {item.get('away', '')}"
        result.append(item)
...
def update_history():
    try:
        ...
        return jsonify({
            "success": True,
            "total": stats.get("total", len(history)),
            "wins": stats.get("wins", 0),
            "losses": stats.get("losses", 0),
            "pushes": stats.get("pushes", 0),
